...
class DrugPrescribedAdapter(
    val context: Context,
    val onItemClick: (Any?, Any?) -> Unit
) : BaseRecyclerViewAdapter<String, DrugPrescribedAdapter.TopListViewHolder>() {


    override fun createItemViewHolder(parent: ViewGroup): TopListViewHolder {
        val itemBinding =
            RawDrugPrescribedBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class TopListViewHolder(private val itemBinding: RawDrugPrescribedBinding) : ViewHolder(itemBinding.root){
